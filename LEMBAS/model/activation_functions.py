...
def MML_activation(x: torch.Tensor, leak: Union[float, int]):
    ...
    shifted_x = 0.5 * (fx - 0.5)  # shift and scale
    mask = torch.lt(shifted_x, 0.0)  # create mask | if shifted_x < 0 --> True
    gated_x = fx + 10 * mask #prevents division by 0 issue on next line
    right_values = 0.5 + torch.div(shifted_x, gated_x)  # Scaling
    fx = mask * (fx - right_values) + right_values #-fx trick from relu | combines original with masked (right) values
    return fx
